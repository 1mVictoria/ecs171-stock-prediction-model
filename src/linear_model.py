...
class LinearRegression32GD:
    ...
    def fit(self, X_train, y_train, X_test, y_test, lr=0.0001, epochs=100, batch_size=32, weight_action=1, weight_hold=1):
        ...
        for epoch in range(epochs):
            ...
            # weighted
            weights = np.where(y_batch == 1, weight_action, weight_hold)
            error = y_pred - y_batch
            # compute the weighted gradient of the loss function
            weighted_error = weights * error
            gradient = 2 / batch_size * X_batch.T.dot(weighted_error)
...
def main(args):
    ...
    # # Weighted Setting (Uncomment for weighted version)
    # counts = Counter(y_train)
    # # num of samples
    # total = sum(counts.values())
    # # 2 for num of classes
    # weight_action = total / (2 * counts[1])
    # weight_hold = total / (2 * counts[0])
